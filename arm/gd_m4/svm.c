...
void read_model_header_hard_code(struct svm_model *model)
{
	...
	if (model->rho == NULL) printf("model->rho Malloc error !!! \n");
	model->rho[0] = 45.508316488952168;
	model->label = Malloc(int, 2);
	if (model->label == NULL) printf("model->label Malloc error !!! \n");
	model->label[0] = 1;
	model->label[1] = -1;
	model->probA = Malloc(double, 1);
	if (model->probA == NULL) printf("model->probA Malloc error !!! \n");
	model->probA[0] = -0.26574469547452156;
	model->probB = Malloc(double, 1);
	model->probB[0] = -0.1356215583183184;
	// prob_density_marks
	model->nSV = Malloc(int, 2);
	if (model->nSV == NULL) printf("model->nSV Malloc error !!! \n");
	model->nSV[0] = 122;
	model->nSV[1] = 126;
...
struct svm_model *svm_load_model_hard_code()
{
	...
	if (model == NULL) printf("model Malloc error !!! \n");
	...
	if (model->sv_coef == NULL) printf("model->sv_coef Malloc error !!! \n");
	int i;
	for (i = 0; i < m; i++)
		model->sv_coef[i] = Malloc(double, l);

	model->SV = Malloc(struct svm_node *, l);
	if (model->SV == NULL) printf("model->SV Malloc error !!! \n");
