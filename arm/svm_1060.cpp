...
bool read_model_header(FILE *fp, svm_model *model)
{
	...
	while (1)
	{
		...
		else if (strcmp(cmd, "gamma") == 0)
		{
			...
			FSCANF(fp, "%s", _buff);
			param.gamma = atof(_buff);
		}
		else if (strcmp(cmd, "coef0") == 0)
		{
			// FSCANF(fp, "%lf", &param.coef0);
			FSCANF(fp, "%s", _buff);
			param.coef0 = atof(_buff);
		}
		...
		else if (strcmp(cmd, "prob_density_marks") == 0)
		{
			...
			for (int i = 0; i < n; i++)
			{
				...
				FSCANF(fp, "%s", _buff);
				model->prob_density_marks[i] = atof(_buff);
			}
		}
...
svm_model *svm_load_model_buff()
{
	// FILE *fp = fopen(model_file_name, "rb");
	FILE *fp = fmemopen(__tools_smartsd_model, __tools_smartsd_model_len, "r");
	if (fp == NULL)
		return NULL;
